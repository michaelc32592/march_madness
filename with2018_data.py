# -*- coding: utf-8 -*-
"""
Created on Sun Mar  3 14:29:04 2019
...
#placeholder for 2018
    ...
    GROUP BY "WTeamID"
    '''
    FY18_wins = pd.read_sql(sql_wins18, conn)
    FY18_wins["TeamID"] = FY18_wins["WTeamID"]
    FY18_wins["Wins"] = FY18_wins["count"]
    ...
    FY18_losses = pd.read_sql(sql_losses18,conn)
    FY18_losses["TeamID"] = FY18_losses["LTeamID"]
    FY18_losses["Losses"] = FY18_losses["count"]
    
    FY18_record = pd.merge(FY18_wins, FY18_losses, on = "TeamID")
    FY18_record = FY18_record[["TeamID","Wins","Losses"]]
    FY18_record["win_pct"] = FY18_record["Wins"]/ (FY18_record["Losses"] + FY18_record["Wins"])
    #FY18_record.to_sql("2018 Record", engine)
    
    #Strength of Schedule
    sql1128 = r'''
SELECT "WTeamID",AVG("win_pct") FROM (
SELECT "RegularSeasonDetailedResults"."Season", "RegularSeasonDetailedResults"."WTeamID", "2018 Record"."TeamID","2018 Record"."win_pct"
FROM "RegularSeasonDetailedResults"
INNER JOIN "2018 Record" ON "RegularSeasonDetailedResults"."LTeamID"="2018 Record"."TeamID"
WHERE "Season" = 2018) AS foo
GROUP BY "WTeamID"
ORDER BY AVG("win_pct") DESC
''' 
    FY2018_sos_w = pd.read_sql(sql1128, conn)
    FY2018_sos_w.columns = ['TeamID','WSoS']
    FY2018_df_2 = pd.merge(FY2018_df, FY2018_sos_w, how = 'left',on = 'TeamID')
    
    sql1129 = r'''
SELECT "LTeamID",AVG("win_pct") FROM (
SELECT "RegularSeasonDetailedResults"."Season", "RegularSeasonDetailedResults"."LTeamID", "2018 Record"."TeamID","2018 Record"."win_pct"
FROM "RegularSeasonDetailedResults"
INNER JOIN "2018 Record" ON "RegularSeasonDetailedResults"."WTeamID"="2018 Record"."TeamID"
WHERE "Season" = 2018) AS foo
GROUP BY "LTeamID"
ORDER BY AVG("win_pct") DESC
'''

    FY2018_sos_l = pd.read_sql(sql1129, conn)
    FY2018_sos_l.columns = ['TeamID','LSoS']
    FY2018_df_3 = pd.merge(FY2018_df_2, FY2018_sos_l, how = 'left',on = 'TeamID')
    
    #FT%
    sql2011 = '''
SELECT "WTeamID",COUNT("EventType") FROM "PlaybyPlayEvents18"
WHERE "ElapsedSeconds" > 2200  AND "WTeamID" = "EventTeamID" AND "EventType" = 'made1_free'
GROUP BY "WTeamID"
ORDER BY COUNT("EventType") DESC
'''

    FY2018_clutch_ft_made = pd.read_sql(sql2011, conn)
...
ORDER BY COUNT("EventType") DESC
'''
    FY2018_clutch_ft_miss = pd.read_sql(sql2012, conn)

    FY2018_clutch_ft = pd.merge(FY2018_clutch_ft_made,FY2018_clutch_ft_miss, on = "WTeamID")
    FY2018_clutch_ft["clutch_FtPct"] = FY2018_clutch_ft['count_x']/(FY2018_clutch_ft['count_x']+ FY2018_clutch_ft['count_y'])
    FY2018_clutch_ft['TeamID'] = FY2018_clutch_ft['WTeamID']
    FY2018_clutch_ft = FY2018_clutch_ft[['TeamID','clutch_FtPct']]
    FY2018_df_4 = pd.merge(FY2018_df_3, FY2018_clutch_ft, how = 'left',on = 'TeamID')
    FY2018_df_4['Season'] = 2018
    
    Final_every_year = Final_every_year.append(FY2018_df_4)
...
#Fitting Logistic Regression to the Training Set
    ...
    classifier = SVC(C = 100, gamma = .0001)
    classifier.fit(X_train, y_train)
...
def NB(X_train, X_test, y_train, y_test):
    ...
    y_pred = classifier.predict(X_test)
    from sklearn.metrics import classification_report, confusion_matrix
    nb_pred = pd.DataFrame(data = y_pred, columns = ["NB"])
    import sklearn
    accuracy = sklearn.metrics.accuracy_score(y_test, y_pred, normalize = True, sample_weight = None)
    d = {'NB': accuracy}
    nb_acc = pd.DataFrame(data = d, index = [0])
    
    filename = 'NB_model.sav'
    pickle.dump(classifier, open(filename, 'wb'))
    
    
    return nb_pred, nb_acc
...
def ensemble():
    ...
    for train_index, test_index in kf.split(X):
        ...
        nb_pred, nb_acc = NB(X_train, X_test, y_train, y_test)
        ...
        df9 = df8.join(nb_pred, how = "outer", lsuffix = "_", rsuffix = "_")
        ...
        df25 = df24.join(nb_acc,how = "outer", lsuffix = "_", rsuffix = "_")
    
        conc_df = conc_df.append(df9, ignore_index = True)
        conc_acc = conc_acc.append(df25, ignore_index = True)
        import statistics
        ...
        X_2 = conc_df[['SVM', 'Random_Forest', 'KNN', 'Log_reg','NB']]
        y_2 = conc_df[['Actual']]
...
def set_up_MarchMadness():
    ...
    final_bracket = bracket_2[['Average Pts','Average PA','Home Record','Away Record','Coach Record','offensive rebounds','offensive rbs allowed',
                                 'avg_to_made','avg_to_allowed','conf win pct','Seed','WSoS','LSos','Clutch Ft Pct']]
    return final_bracket,bracket_3
        
def main(model, bracket2):   
    #Get seeds
    #model_df = set_up_MarchMadness()
    ...
    dfs = [log_reg_df, KNN_df, SVM_df, RF_df]
    ...
    absolute_final_df = absolute_final_df[['Final Prediction','Team1ID','Team2ID','Slot']]
    round1_lg, width = absolute_final_df.shape
    
    return absolute_final_df, bracket2

def round_2():  
    final_bracket,bracket2 = set_up_MarchMadness() 
    ...
    for i in range(0,35):
        if absolute_final_df.iloc[i,0] == 1:
            absolute_final_df.iloc[i,4] = absolute_final_df.iloc[i,1]
            
        
        if absolute_final_df.iloc[i,0] == 0:
            absolute_final_df.iloc[i,4] = absolute_final_df.iloc[i,2]
    ...
    round2_df20 = round2_df20[['Team1ID','Team2ID','Round','TeamName1','TeamName2']]
    ...
    #run through model
    return final_bracket_round2, round2_df13
            

def round_3():
    #figure out the bracket 2 (make sure every round has one which is used to join)
    final_bracket_round2, bracket2 = round_2()
    ...
    for i in range(0,16):
        if round_2_results.iloc[i,0] == 1:
            round_2_results.iloc[i,4] = round_2_results.iloc[i,1]
            
        
        if round_2_results.iloc[i,0] == 0:
            round_2_results.iloc[i,4] = round_2_results.iloc[i,2]
    ...
    round3_df20 = round3_df20[['Team1ID','Team2ID','Round','TeamName1','TeamName2']]
    ...
    #run through model
    return final_bracket_round3, round3_df13

def round_4():
    #figure out the bracket 2 (make sure every round has one which is used to join)
    final_bracket_round3, bracket3 = round_3()
    ...
    for i in range(0,8):
        if round_3_results.iloc[i,0] == 1:
            round_3_results.iloc[i,4] = round_3_results.iloc[i,1]
            
        
        if round_3_results.iloc[i,0] == 0:
            round_3_results.iloc[i,4] = round_3_results.iloc[i,2]
    ...
    round4_df4 = round4_df4[['Slot_x_y','Winner_x','Winner_y']]
    ...
    round4_df20 = round4_df20[['Team1ID','Team2ID','Round','TeamName1','TeamName2']]
    ...
    return final_bracket_round4, round4_df13

def round_5():
    #figure out the bracket 2 (make sure every round has one which is used to join)
    final_bracket_round4, bracket4 = round_4()
    ...
    for i in range(0,4):
        if round_4_results.iloc[i,0] == 1:
            round_4_results.iloc[i,4] = round_4_results.iloc[i,1]
            
        
        if round_4_results.iloc[i,0] == 0:
            round_4_results.iloc[i,4] = round_4_results.iloc[i,2]
    ...
    round5_df4 = round5_df4[['Slot_x_y','Winner_x','Winner_y']]
    ...
    round5_df20 = round5_df20[['Team1ID','Team2ID','Round','TeamName1','TeamName2']]
    ...
    return final_bracket_round5, round5_df13

def round_6():
    final_bracket_round5, bracket5 = round_5()
    ...
    for i in range(0,2):
        if round_5_results.iloc[i,0] == 1:
            round_5_results.iloc[i,4] = round_5_results.iloc[i,1]
            
        
        if round_5_results.iloc[i,0] == 0:
            round_5_results.iloc[i,4] = round_5_results.iloc[i,2]
    ...
    round6_df4 = round6_df4[['Slot_x_y','Winner_x','Winner_y']]
    ...
    round6_df20 = round6_df20[['Team1ID','Team2ID','Round','TeamName1','TeamName2']]
    ...
    return final_bracket_round6, round6_df13

def Winner():

    final_bracket_round6, bracket6 = round_6()
    ...
    for i in range(0,1):
        if round_6_results.iloc[i,0] == 1:
            round_6_results.iloc[i,4] = round_6_results.iloc[i,1]
            
        
        if round_6_results.iloc[i,0] == 0:
            round_6_results.iloc[i,4] = round_6_results.iloc[i,2]
    ...
    round_6_results = round_6_results[['Team1ID','Team2ID','Round','TeamName1','TeamName2']]
    final_deliverable = round_6_results.append(round6_df20)
    final_deliverable.to_excel("2018_results_prediction.xlsx")
